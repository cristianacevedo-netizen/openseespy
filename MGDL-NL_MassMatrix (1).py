...
# printA('-file', "Outputs/Matrices/M.out", '-ret','Mass')
Mlist=printA('-ret')

nDOF = int(len(Mlist)**0.5)

M = np.array(Mlist).reshape((nDOF, nDOF))

plt.spy(M, markersize=1)

plt.xlabel("Grados de libertad (columnas)", fontsize=12)
plt.ylabel("Grados de libertad (filas)", fontsize=12)
plt.xticks(np.arange(0, nDOF, int(nDOF/10)))
plt.yticks(np.arange(0, nDOF, int(nDOF/10)))
